...
bot.command('ban', async ctx => {
  ...
  if (!userId) return ctx.reply('⚠️ Lütfen bir mesaja reply edin veya @kullanıcıadı / ID belirtin!');
...
bot.command('warn', async ctx => {
  ...
  try {
    ...
    if (user.warnings >= 3) {
      ...
      await ctx.banChatMember(ctx.chat.id, userId).catch(e => console.error('Telegram ban hatası:', e));
...
bot.on('text', async ctx => {
  ...
  const muteMinutes = ctx.session.muteMinutes || 30;
  ...
  try {
    if (action === 'ban') {
      ...
      await UserService.muteUser(targetId, muteMinutes, reason, admin);
      await ctx.reply(`🔇 ${muteMinutes} dakika susturuldu. Sebep: ${reason}`);
